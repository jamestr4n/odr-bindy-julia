...
# ╔═╡ 950b63d3-f76e-429e-a99f-2efc9861e074
using DifferentialEquations, DiffEqFlux, Flux

# ╔═╡ e3b88485-61c5-4879-8be8-31086ac8e916
using AdvancedHMC, MCMCChains, Plots

# ╔═╡ 8d294c7c-cfd2-4046-aa62-197fd4a89de3
using ComponentArrays, Random

# ╔═╡ 5584c3f0-9507-11f1-962b-d9f5a87acfc8
md"""
# SciML Demo
"""

# ╔═╡ 4dd104b7-ff72-4f8d-8a79-78350cb4f31c
md"""
## Step 1: Set Up
"""

# ╔═╡ 64dd10a7-9e91-4794-812e-cf15a84b238e
u0 = [2.0; 0.0]

# ╔═╡ f5995be0-bf6a-4f2f-ba20-26817116d9c3
datasize = 40

# ╔═╡ ce68ba13-4780-46dd-9b8e-0c389e005988
t_begin = 0.0

# ╔═╡ debc17ca-7480-4a34-b693-401eaff71bf2
t_end = 1.0

# ╔═╡ 6514973a-8b3c-4e9e-b9b0-75566635c653
tspan = (t_begin, t_end)

# ╔═╡ 30f5ae51-2fd0-41bf-950b-20b8aa864029
tsteps = range(t_begin, t_end, length = datasize)

# ╔═╡ 3771ea36-f8b4-45cd-a39f-036b25a6eee0
function trueODEfunc(du, u, p, t)
	true_A = [-0.1 2.0; -2.0 -0.1]
	du .= ((u .^ 3)' * true_A)'
end

# ╔═╡ aa97d733-848f-4025-b445-f7298936cf6d
prob_trueode = ODEProblem(trueODEfunc, u0, tspan)

# ╔═╡ 49e2dfb6-7ec0-49c6-bf17-f2a29293da2a
ode_data = Array(
	solve(prob_trueode, Tsit5(), saveat = tsteps)
)

# ╔═╡ 194d1cca-b0d2-4422-81e0-92c91df2d167
md"""
## Plot Data
"""

# ╔═╡ 765d6fb0-1dea-4d43-b925-73b28ce262d7
plot1 = scatter(tsteps, ode_data[1, :],
	color = :red,
	label = "Data: Var1",
	xlabel = "t",
	title = "Spiral Neural ODE (Time Series)"
)

# ╔═╡ dad41b81-5499-4ad4-8934-53920b8849d3
scatter!(plot1, tsteps, ode_data[2, :],
	color = :blue,
	label = "Data: Var2"
)

# ╔═╡ 2805993d-9bd0-48fb-b92e-cad6b4761503
plot2 = scatter(ode_data[1, :], ode_data[2, :],
	color = :red,
	label = "Data",
	xlabel = "Var1",
	ylabel = "Var2",
	title = "Spiral Neural ODE (Phase Space)",
	lims = (-3, 3),
	widen = true,
	aspect_ratio = 1
)

# ╔═╡ 1c2394bd-6978-482b-900f-19a5e3056ebe
md"## Step 2: Define the Neural ODE Architecture"

# ╔═╡ 52516502-9342-4398-b889-561fa80aca4f
dudt2 = Lux.Chain(x -> x .^ 3,
	Lux.Dense(2, 50, tanh),
	Lux.Dense(50, 2)) |> Lux.f64

# ╔═╡ 2e267735-6a70-4aef-87a7-d49d03dffb3a
prob_neuralode = NeuralODE(
	dudt2, tspan, Tsit5(), saveat = tsteps
)

# ╔═╡ 3250be22-d030-4c90-9bb6-e52858e25dec
rng = Random.default_rng()

# ╔═╡ a69e0dcd-e851-4b3a-8a4a-6f84b4bcadad
begin
	p, st = Lux.setup(rng, dudt2)
	p = Float64.(ComponentArray(p))
end

# ╔═╡ e30c8810-3917-4ffe-b432-b42d905527ad
p_flat = Vector(p)

# ╔═╡ 33490228-0e49-4da7-bb36-66fe804bbb3d
md"## Step 3: Define Loss Function for Neural ODE"

# ╔═╡ 71fce1a1-fa30-495d-85f9-3e1a27cc7067
function predict_neuralode(p)
	Array(prob_neuralode(u0, p, st)[1])
end

# ╔═╡ f3293ed6-2123-4312-a897-88bb97e105e0
...
# ╔═╡ ecb16607-d3fd-47aa-b855-5211376e1aa9
md"""
## Step 4: Integrate Bayesian Estimation Workflow (as prescibed by the AdvancedHMC interface) with the NeuralODE defined above
"""

# ╔═╡ 952dab73-eb15-487f-9e4f-91b2468bdb54
loss(theta) = begin
	theta_ca = ComponentArray(theta, getaxes(p))
	-sum(abs2, ode_data .- predict_neuralode(theta_ca)) - sum(theta .* theta)
end

# ╔═╡ b048892f-4fe3-45ab-9a61-82d53791cd54
function dloss_dtheta(theta)
	x, lambda = Flux.Zygote.pullback(loss, theta)
	grad = first(lambda(1))
	return x, grad
end

# ╔═╡ 45e313ad-b8f2-4142-884c-bb3c0729cbe9
metric = DiagEuclideanMetric(length(p_flat))

# ╔═╡ 9508d03c-b7cb-4397-a8a8-cbdb4b51fac0
hamiltonian = Hamiltonian(metric, loss, dloss_dtheta)

# ╔═╡ 4e4ef0da-e3c3-4a49-8445-a961dc7a3835
md"Note: finding the integrator may take a few minutes."

# ╔═╡ def72381-f8cc-46c5-8ccc-6fc092079cfe
integrator = Leapfrog(
	find_good_stepsize(hamiltonian, p_flat)
)

# ╔═╡ 95fa63ac-1a15-448b-8fdf-f93de42466b5
kernel = HMCKernel(Trajectory{MultinomialTS}(integrator, GeneralisedNoUTurn()))

# ╔═╡ 64b349db-0159-482e-b31c-ae15b75ac0c8
adaptor = StanHMCAdaptor(
	MassMatrixAdaptor(metric),
	StepSizeAdaptor(0.45, integrator)
)

# ╔═╡ e9939f56-dbfe-4efb-9184-880335886ec7
md"Warning: drawing 500 samples may take over 30 minutes to run."

# ╔═╡ c9e56b43-ca65-4c87-b5dc-fae32cbaa48e
n_samples = 50

# ╔═╡ 5ebd3a35-7044-4115-b52a-43633553fd18
samples, stats = sample(hamiltonian, kernel, p_flat,
	n_samples, adaptor, n_samples;
	progress = true
)

# ╔═╡ ff4c11cd-f346-461a-af83-729827ae06fd
md"## Step 5: Analysis"

# ╔═╡ d3d02acc-b1d8-4950-a426-d2a85a56f1f5
samples_hcat = hcat(samples...)

# ╔═╡ f58931db-0004-48ea-ae9a-9b1bc8fc71a8
md"### Plot 1: Time Series"

# ╔═╡ 77ae8f89-d171-403f-ab9c-4f799a3c0745
for k in 1:300
	theta_flat = samples_hcat[:, 10:end][:, rand(1:40)]
	theta_ca = ComponentArray(theta_flat, getaxes(p))
	resol = predict_neuralode(theta_ca)
	plot!(plot1, tsteps, resol[1, :],
		alpha = 0.04,
		color = :red,
		label = ""
	)
	plot!(plot1, tsteps, resol[2, :],
		alpha = 0.04,
		color = :blue,
		label = ""
	)
...
# ╔═╡ d0304857-a078-442f-a278-14115291859a
plot1

# ╔═╡ 76274887-ae4a-4cee-855f-7c5b77c35253
losses = map(x -> loss_neuralode(ComponentArray(x, getaxes(p)))[1],
	eachcol(samples_hcat)
)

# ╔═╡ afc4c5fa-09e6-4d90-b5fd-d1c36a04b9af
idx = findmin(losses)[2]

# ╔═╡ 3d5ae802-0e22-4d6c-8b8a-dc9785a570d5
prediction = predict_neuralode(
	ComponentArray(samples_hcat[:, idx], getaxes(p))
)

# ╔═╡ 04e025a0-4a16-490b-b780-9c4be768466c
plot!(plot1, tsteps, prediction[1, :],
	color = :black,
	linewidth = 2,
	label = ""
)

# ╔═╡ dbb5a71a-de7f-4fb3-bd8b-9909f2bd3c3d
plot!(plot1, tsteps, prediction[2, :],
	color = :black,
	linewidth = 2,
...
manifest_format = "2.0"
project_hash = "b3a632ecdcf7ce8dc9771ee2f49251b79bade733"
...
git-tree-sha1 = "5d2ce92a02cdd73e15ccd87a9759fd57e33b6862"
...
git-tree-sha1 = "952283a79f28d63e437b16cd3776b253b116e21c"
...
[[deps.DiffEqFlux]]
deps = ["ADTypes", "Boltz", "ChainRulesCore", "ConcreteStructs", "Distributions", "LinearAlgebra", "Lux", "LuxCore", "LuxLib", "PrecompileTools", "Random", "Reexport", "SciMLBase", "SciMLSensitivity", "Setfield", "Static"]
git-tree-sha1 = "2894882df1141bb499a4f870ed9ade8cfa30642c"
...
git-tree-sha1 = "929e7d46e55395e956d51e2fabc15ccf8ca1f36b"
...
git-tree-sha1 = "ce2e8d014c4dc9e05730c3a6f0ef4394abac1049"
...
git-tree-sha1 = "83e8020350c1c1e37f24b05bf6aeaba0b3faad9e"
...
deps = ["ADTypes", "CommonSolve", "DiffEqBase", "FastBroadcast", "LinearAlgebra", "MacroTools", "MuladdMacro", "OrdinaryDiffEqCore", "OrdinaryDiffEqDifferentiation", "OrdinaryDiffEqNonlinearSolve", "PrecompileTools", "Preferences", "RecursiveArrayTools", "Reexport", "SciMLBase", "TruncatedStacktraces"]
git-tree-sha1 = "ca7b9697d367aeb6a2bf47f647957ec8176c5c27"
...
git-tree-sha1 = "d606a9604a80ac92dcd86d34c17e4b23783c2dc3"
